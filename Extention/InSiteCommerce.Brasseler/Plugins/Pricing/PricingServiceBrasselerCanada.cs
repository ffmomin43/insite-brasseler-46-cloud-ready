...
{
    [DependencyName("BrasselerCanada")]
    public class PricingServiceBrasselerCanada : PricingServiceGeneric
    {
      ...
      : base(unitOfWorkFactory, currencyFormatProvider, orderLineUtilities, pricingServiceFactory, perRequestCacheManager,priceMatrixUtilities, pricingSettings)
        ...
        public string GetPriceListAmount(string pricebasis)
        {
            ...
            {
                var priceListNo = decimal.Zero;
                switch (pricebasis)
                {
                    case "P1":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price1;
                        break;
                    case "P2":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price1;
                        break;
                    case "P3":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price2;
                        break;
                    case "P4":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price3;
                        break;
                    case "P5":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price4;
                        break;
                    case "P6":
                        priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price5;
                        break;
                }
                ...
                //    if (defaultPriceList != null && !string.IsNullOrEmpty(defaultPriceList.Value) && decimal.TryParse(defaultPriceList.Value, out priceListNo))
                //        priceList = this.Product.CustomProperties.Where(p => p.Name.EqualsIgnoreCase("Price" + priceListNo)).FirstOrDefault();
                //}

                if ((pricebasis.EqualsIgnoreCase("P1") || pricebasis.EqualsIgnoreCase("L")) && this.ShipTo != null)
                {
                    var defaultPriceList = this.ShipTo.CustomProperties.Where(p => p.Name.EqualsIgnoreCase("DefaultPriceList")).FirstOrDefault();
                    if (defaultPriceList != null && !string.IsNullOrEmpty(defaultPriceList.Value) && decimal.TryParse(defaultPriceList.Value, out priceListNo))
                    {
                        switch (priceListNo.ToString())
                        {
                            case "1":
                                priceListNo = UnitOfWork.GetRepository<PriceMatrixBrasseler>().GetTable().Where(x => x.ProductId == this.Product.Id).FirstOrDefault().Price1;
                                break;
                ...
                if (priceListNo != decimal.Zero)
                    return priceListNo.ToString();
                else
                    return null;
            }
            else
